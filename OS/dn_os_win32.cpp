...
DN_API DN_OSDateTime DN_OS_DateUnixTimeSToDate(DN_U64 time)
{
  ...
  DN_U64   win32_time    = 116'444'736'000'000'000 + (time * 10'000'000);
  SYSTEMTIME sys_time      = {};
  FILETIME   file_time     = {};
...
{
  DN_Assert(g_dn_os_core_);
  if (!buffer || size < 0 || !g_dn_os_core_->win32_bcrypt_init_success)
    return false;

  if (size == 0)
    return true;

  long gen_status = BCryptGenRandom(g_dn_os_core_->win32_bcrypt_rng_handle, DN_CAST(unsigned char *) buffer, size, 0 /*flags*/);
  if (gen_status != 0) {
...
{
  DN_Assert(g_dn_os_core_ && g_dn_os_core_->win32_qpc_frequency.QuadPart);
  DN_U64 result = g_dn_os_core_->win32_qpc_frequency.QuadPart;
  return result;
}
...
static DN_U64 DN_Win_FileTimeToSeconds_(FILETIME const *time)
{
...
DN_API bool DN_OS_DirIterate(DN_Str8 path, DN_OSDirIterator *it)
{
  ...
  if (result) {
    int size      = DN_Win_Str16ToStr8Buffer(wide_it.file_name, it->buffer, DN_ArrayCountU(it->buffer));
...
// NOTE: R/W Stream API ////////////////////////////////////////////////////////////////////////////
DN_API DN_OSFile DN_OS_FileOpen(DN_Str8 path, DN_OSFileOpen open_mode, uint32_t access, DN_OSErrSink *err)
{
  DN_OSFile result = {};
...
DN_API void DN_OS_MutexDeinit(DN_OSMutex *mutex)
{
  ...
  DN_Memset(mutex->win32_handle, 0, DN_ArrayCountU(mutex->win32_handle));
}
...
DN_API void DN_Win_ThreadSetName(DN_Str8 name)
{
  ...
  if (g_dn_os_core_->win32_set_thread_description) {
    DN_Str16 name16 = DN_Win_Str8ToStr16(tmem.arena, name);
    g_dn_os_core_->win32_set_thread_description(GetCurrentThread(), (WCHAR *)name16.data);
...
DN_API DN_WinError DN_Win_ErrorCodeToMsgAlloc(uint32_t error_code)
{
  ...
  if (error16.size)
    result.msg = DN_Win_Str16ToStr8FromHeap(error16);
  if (error16.data)
